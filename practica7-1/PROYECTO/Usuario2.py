    ...
    def run(self):
        ...
        def archivo():# we don't want a full GUI, so keep the root window from appearing
            ...
            try:
                ...
                try:
                    s.sendto("2".encode(), Ips.enviar2)
                    s.sendto(filename[0].encode(), Ips.enviar2)
                    print("Connected with Server")
                    print(os.stat(filename[0]).st_size)
                    s.sendto(str(os.stat(filename[0]).st_size).encode(),Ips.enviar2)
                    f = open(filename[0],'rb')
                    l = f.read(1024)
                    while (l):
                        s.sendto(l, Ips.enviar2)
                        l = f.read(1024)
                    f.close()
                    print('Done sending')
                    sum = s.recv(1024).decode()
                    ...
                    s.sendto("2".encode(), Ips.enviar3)
                    s.sendto(filename[0].encode(), Ips.enviar3)
                    print("Connected with Server")
                    print(os.stat(filename[0]).st_size)
                    s.sendto(str(os.stat(filename[0]).st_size).encode(),Ips.enviar3)
                    f = open(filename[0],'rb')
                    l = f.read(1024)
                    while (l):
                        s.sendto(l, Ips.enviar3)
                        l = f.read(1024)
                    f.close()
                    print('Done sending')
                    sum = s.recv(1024).decode()
