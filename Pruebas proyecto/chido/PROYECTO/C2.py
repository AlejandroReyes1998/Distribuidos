	...
	def run(self):
		...
		while True:
			...
			if(message.startswith('RELOJ')):
				...
				print('llego de C1: ',message.split('-')[1],'  es: ',hora)
				sock.sendto(hora.encode(),addr)
				ajitzi = datetime.datetime.strptime(message, '%H:%M:%S')
				flag = 0
				if datetime.datetime.strptime(self.r1.hora, '%H:%M:%S') > ajitzi:
					flag = 1
				try:
			...
			elif(message.startswith('C3')):
				hora = self.r1.hora.strftime('%H:%M:%S')
				print('llego de C3: ',message.split('-')[1],'  es: ',hora)
				sock.sendto(hora.encode(),addr)
				ajitzi = datetime.datetime.strptime(message, '%H:%M:%S')
				flag = 0
				if datetime.datetime.strptime(self.r1.hora, '%H:%M:%S') > ajitzi:
	...
	def run(self):
		...
		while True:
			...
			if (data.decode().startswith('respaldo')):
				...
				if(prioridades['Coor2'] > prioridades['Coor3']):
					try:
						connection = mysql.connector.connect(host='localhost',
														database='base1',
														user='root',
					...
					if len(res) > 0:
						...
						while tam>0:
							if tam > 1024:
								...
								tam = 0
						s.close()
			elif (data.decode().startswith('desperte-3')):
				if(prioridades['Coor2'] > prioridades['Coor1']):
					try:
						connection = mysql.connector.connect(host='localhost',
														database='base1',
														user='root',
					...
					if len(res) > 0:
						...
						data = json.dumps(tabla)	
						print(data)
						s.sendto("refresh".encode(),Ips.c3)
						tam = len(data.encode())
						print(tam)
						s.sendto(str(tam).encode(),Ips.c3)
						while tam>0:
							if tam > 1024:
								cadena = data[0:1023]
								data = data[1023:]
								tam = tam-1024
								s.sendto(cadena.encode(),Ips.c3)
							else:
								s.sendto(data.encode(),Ips.c3)
								tam = 0
	...
	def run(self):
		while True:
			try:
				...
				prioridades = Ips.prioridades
				if(prioridades['Coor2'] > prioridades['Coor1'] > prioridades['Coor3']):
					c1 = Comunicador1(r1)
					c1.start()
				else:
					while True:
						try:
							...
							message = self.root.hora.strftime('%H:%M:%S')
							starting_point = time.time()
							sock2.sendto("C2".encode(),Ips.respaldo)
...
try:
	s = socket.socket(socket.AF_INET,   socket.SOCK_DGRAM)
	s.sendto("desperte-2".encode(),Ips.c1)
	s = socket.socket(socket.AF_INET,   socket.SOCK_DGRAM)
	s.sendto("desperte-2".encode(),Ips.c3)
	s.close()
except:
	pass
root.mainloop()
