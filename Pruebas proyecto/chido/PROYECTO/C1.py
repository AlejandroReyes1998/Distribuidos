	...
	def run(self):
		...
		while True:
			...
			if (data.decode().startswith('respaldo')):
				...
				if(prioridades['Coor1'] > prioridades['Coor3']):
					try:
						connection = mysql.connector.connect(host='localhost',
														database='base1',
														user='root',
				...
				if(prioridades['Coor1'] > prioridades['Coor2']):
					try:
						connection = mysql.connector.connect(host='localhost',
														database='base1',
														user='root',
	...
	def run(self):
		while True:
			try:
				...
				prioridades = Ips.prioridades
				if(prioridades['Coor1'] > prioridades['Coor2'] > prioridades['Coor3']):
					c1 = Comunicador1(r1)
					c1.start()
				else:
					while True:
						try:
							...
							message = self.root.hora.strftime('%H:%M:%S')
							starting_point = time.time()
							sock2.sendto("C1".encode(),Ips.respaldo)
...
try:
	s = socket.socket(socket.AF_INET,   socket.SOCK_DGRAM)
	s.sendto("desperte-1".encode(),Ips.c2)
	s = socket.socket(socket.AF_INET,   socket.SOCK_DGRAM)
	s.sendto("desperte-1".encode(),Ips.c3)
	s.close()
except:
	pass
root.mainloop()
